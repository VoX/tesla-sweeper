...
describe('classifyWeek — imminent-opposite', () => {
  it('detects opposite-side sweep BEFORE user-side event', () => {
    // VoX's actual schedule: Thu ODD, Fri EVEN. User on EVEN, today Wed.
    // Naive flip would put them on ODD before Thu morning sweep.
    const p = classifyWeek({
  ...
  it('does NOT trigger when opposite is today (opposite-already-swept wins)', () => {
    const p = classifyWeek({
  ...
  it('does NOT trigger when opposite is AFTER user-side event (tight-flip wins)', () => {
    const p = classifyWeek({
...
describe('formatPlanDM — message smoke tests', () => {
  ...
  it('imminent-opposite says wait then move that evening', () => {
    const p = classifyWeek({
      events: [ev('2026-05-07', 'odd'), ev('2026-05-08', 'even'), ev('2026-05-21', 'odd')],
      carSide: 'even', todayET: '2026-05-06',
    });
    const m = formatPlanDM({ vehicleName: 'T', address: 'A', plan: p });
    expect(m).toContain('ODD sweeps Thu morning');
